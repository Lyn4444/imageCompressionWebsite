...
class CNN():
    ...
    def load_vgg_weights(self):
        with open(hyper.vgg_weights) as f:
            self.pretrained_weights = pk.load(f)
    ...
    def conv2d_depth_or_not(self, input_, name, nonlinearity=None):
        with tf.compat.v1.variable_scope(name) as scope:
            ...
            if hyper.fine_tuning and name not in ['conv6', 'conv6_1', 'depth']:
                ...
                W_initializer = tf.compat.v1.truncated_normal_initializer(stddev=hyper.stddev)
                b_initializer = tf.constant_initializer(0.0)

            conv_weights = tf.compat.v1.get_variable("W", shape=W_shape, initializer=W_initializer)
            conv_biases = tf.compat.v1.get_variable("b", shape=b_shape, initializer=b_initializer)

            if name == 'depth':
                # learn different filter for each input channel
                # thus the number of input channel has to be reduced
                conv = tf.compat.v1.nn.depthwise_conv2d_native(input_, conv_weights, [1, 1, 1, 1], padding='SAME')
    ...
    def build(self, image):
        ...
        with tf.compat.v1.variable_scope("GAP"):
            gap_w = tf.compat.v1.get_variable("W", shape=cnn_param.layer_shapes['GAP/W'],
                                              initializer=tf.random_normal_initializer(stddev=hyper.stddev))
    ...
    def get_classmap(self, class_, conv_last):
        with tf.compat.v1.variable_scope("GAP", reuse=True):
            class_w = tf.gather(tf.transpose(tf.compat.v1.get_variable("W")), class_)
            class_w = tf.reshape(class_w, [-1, cnn_param.last_features, 1])
        conv_last_ = tf.compat.v1.image.resize_bilinear(conv_last, [hyper.image_h, hyper.image_w])

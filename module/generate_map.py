...
images_tf = tf.compat.v1.placeholder(tf.float32, [None, hyper.image_h, hyper.image_w, hyper.image_c], name="images")
class_tf = tf.compat.v1.placeholder(tf.int64, [None], name='class')
...
with tf.compat.v1.Session(config=config) as sess:
    tf.compat.v1.train.Saver().restore(sess, hyper.model_path)
